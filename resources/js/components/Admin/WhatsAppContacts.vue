        ...
        <v-card-text>
          <v-form @submit.prevent="save">
            ...
            <v-select
              ...
              label="Type"
              required
              hint="Contact type for separate call tracking"
              persistent-hint
            />
            <v-text-field
...
export default {
  ...
  data() {
    return {
      ...
      editing: false,
      currentId: null,
      form: { name: '', phone: '', type: 'junior', counter: 1 },
  ...
  methods: {
    ...
    fetch() { this.loading = true; api.get('/whatsapp-contacts').then(res => this.items = res.data).finally(() => this.loading = false); },
    openCreate() { this.editing = false; this.currentId = null; this.form = { name: '', phone: '', type: 'junior', counter: 1 }; this.dialog = true; },
    openEdit(item) {
      this.editing = true;
    ...
    save() {
      ...
      const req = this.editing
        ? api.put(`/whatsapp-contacts/${this.currentId}`, payload)
        : api.post('/whatsapp-contacts', payload);
    ...
    confirmDelete() {
      ...
      api.delete(`/whatsapp-contacts/${this.confirm.item.id}`)
