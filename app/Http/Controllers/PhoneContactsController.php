...
class PhoneContactsController extends BaseController
{
    ...
    {
        $this->middleware('auth:sanctum')->except(['index', 'show', 'nextPhoneNumber', 'nextJuniorPhoneNumber', 'nextSeniorPhoneNumber', 'recordPhoneNumber']);
    ...
    public function nextPhoneNumber()
    {
        ...
        if ($lastRecord) {
            ...
            if ($lastContact) {
                ...
                $recentRecords = PhoneRecord::orderBy('id', 'desc')
                    ->take($maxCallsForContact)
                    ->pluck('phone_contacts_id')
                    ->toArray();
                $consecutiveCalls = $recentRecords === array_fill(0, count($recentRecords), $lastContact->id) ? count($recentRecords) : 0;
    ...
    /**
     * Get the next junior phone number.
     */
    ...
    /**
     * Get the next senior phone number.
     */
